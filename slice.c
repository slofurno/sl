...
void free_slice(slice *s) { free(s->bytes); }
...
void slice_appendn(slice *self, char *str, size_t n) { appendn(self, str, n); }
...
int slice_printf(slice *self, const char *format, ...) {
  ...
  while (1) {
    ...
    va_copy(args, ap);
    len =
        vsnprintf(self->bytes + self->len, self->cap - self->len, format, args);
